...
// Production kernel SHA-256: d38cad80d7fc15bd27dcf6174a7e64452fdacff7ea1f6b3e756a3e4dd846d9b2
 ...
 */
export function createQueryStacker(api, { batchSize = 32, queryThreshold = 2, onError = console.error, onGroupCreated, validateGroupIds } = {}) {
  ...
  async function branch(root) {
    ...
    for (let head = 0; head < result.length; head++) {
      ...
      for (let start = 0; start < ids.length; start += batchSize) {
        ...
        const tabs = await Promise.all(chunk.map(id => read(id, root.windowId)));
  ...
  async function attachSource(source, anchor) {
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        const [parent, member] = await Promise.all([read(source.id, source.windowId), read(anchor.id, source.windowId)]);
  ...
  async function join(source, ids) {
    ...
    try {
      ...
      for (const id of ids.toReversed()) {
        for (let attempt = 0; attempt < 2; attempt++) {
          try {
            const [parent, child] = await Promise.all([read(source.id, source.windowId), read(id, source.windowId)]);
  ...
  async function apply(jobs) {
    ...
    const [source, ...tabs] = await Promise.all([
      read(first.sourceTabId, first.windowId), ...jobs.map(job => read(job.tabId, job.windowId)),
    ]);
        ...
        try {
          try {
            ...
            if (error instanceof UnconfirmedCreationError && !created) {
              const opener = await read(batch[0].sourceTabId, windowId);
